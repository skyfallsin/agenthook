...
export function createWorkerManager({
  ...
  command = 'pi', spawnProcess = spawn, request = fetch, onDeliveryError = () => {},
} = {}) {
  const jobs = new Map();
  let closed = false;
  const view = job => ({ id: job.id, topic: job.topic, status: job.status, delivery: job.delivery,
    model: WORKER_MODEL, thinking: 'medium', pid: job.child?.pid ?? null, report: job.report });
  const save = job => fs.writeFileSync(path.join(job.dir, 'status.json'), JSON.stringify(view(job), null, 2), { mode: 0o600 });
  const get = id => {
  ...
  return {
    async start({ task, cwd, topic, url, token, signal, beforeSpawn = () => {} }) {
      ...
      const args = ['--mode', 'json', '-p', '--no-session', '--model', WORKER_MODEL, '--thinking', 'medium',
        '--exclude-tools', 'agenthook,agenthook_subagent,subagent,goal_delegate', '--append-system-prompt', GUIDANCE];
      const job = { id, dir, report, topic, status: 'starting', delivery: 'pending', child: null };
      jobs.set(id, job);
      let finalMessage;
      let ended = false;
      let buffer = '';
      ...
      const finalize = async (code, spawnError = false) => {
        ...
        try {
          ...
          const response = await request(`${url}/v1/webhooks/${encodeURIComponent(topic)}`, {
            ...
            body: JSON.stringify({ kind: 'subagent', workerId: id, status: job.status, report, model: WORKER_MODEL, thinking: 'medium' }),
          });
