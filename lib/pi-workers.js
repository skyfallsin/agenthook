...
const GOAL_BINDING_KEYS = ['PI_GOAL_ID', 'PI_GOALS_STORE', 'PI_CONTEXT_WINDOW_ID', 'PI_PARENT_CONTEXT_WINDOW_ID', 'PI_SPRITE_CONTEXT_CAPABILITY_FILE', 'PI_GOAL_ROLE'];
...
export function createWorkerManager({
  ...
  return {
    async start({ task, title, cwd, topic, url, token, signal, beforeSpawn = () => {}, goalBinding, onRegistered, onComplete }) {
      ...
      if (goalBinding !== undefined && (!goalBinding || typeof goalBinding !== 'object' || Object.entries(goalBinding).some(([key, value]) => !GOAL_BINDING_KEYS.includes(key) || typeof value !== 'string' || !value))) {
        throw new Error('Goal binding must contain only non-empty explicit Pi goal environment values');
      }
      if (onRegistered !== undefined && typeof onRegistered !== 'function') throw new Error('onRegistered must be a function');
      ...
      for (const key of ['AGENTHOOK_TOPIC', 'AGENTHOOK_TOKEN', 'PI_SESSION_ID', 'PI_SESSION_FILE', 'PI_MODEL', 'PI_PROVIDER', 'PI_REASONING_LEVEL', ...GOAL_BINDING_KEYS]) delete env[key];
      if (goalBinding) Object.assign(env, goalBinding);
      const excludedTools = goalBinding
        ? 'agenthook,agenthook_subagent,subagent,legacy_subagent,spawn'
        : 'agenthook,agenthook_subagent,subagent,legacy_subagent,goal_delegate';
      const args = ['--mode', 'json', '-p', ...(goalBinding ? [] : ['--no-session']), '--model', WORKER_MODEL, '--thinking', 'medium',
        '--exclude-tools', excludedTools, '--append-system-prompt', GUIDANCE];
      const job = { id, title: workerTitle(title, task), dir, report, topic, status: 'starting', delivery: goalBinding ? 'direct' : 'pending', child: null, onComplete };
      jobs.set(id, job);
      onRegistered?.(view(job));
      let finalMessage;
      let ended = false;
      let buffer = '';
      let finishResolve;
